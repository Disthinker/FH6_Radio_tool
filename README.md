...
修复点击“③ 最终生成”或“保存并重新生成 XML”后窗口未响应的问题。

原因是最终生成会执行大量磁盘 I/O、音频复制、音量匹配和 XML 输出。旧版本在主 UI 线程直接执行这些任务，音频数量较多或文件较大时，Windows 会把窗口标记为“未响应”。

v3.7.1 起，最终生成改为后台线程执行：
...
这可以避免用户在音频复制、音量匹配、生成 `fmod_ready_wav` 时误以为程序已经卡住。

说明：部分步骤，例如大 WAV 文件的音量匹配，无法精确显示每一首文件的真实百分比，因此进度条会结合“阶段进度 + 心跳前进”显示。只要已用时仍在变化，就说明后台任务仍在工作。


## v3.7.3 修复

修复 `Creating fmod_ready_wav` 阶段长时间停在 90% 附近的问题。

v3.7.2 的进度条主要是 UI 层面的阶段提示，真正耗时的“复制 Extract 模板、逐首替换音频、逐首音量匹配”仍然包在一个大函数里，因此用户会看到进度提前接近 90%，然后长时间不动。

v3.7.3 起，进度回调已下沉到实际生成逻辑中：

```text
复制原 Extract 模板
逐首替换 sound_x.wav
逐首进行音量匹配
写入 volume_match_report
整理最终 output
```
...
```text
Replacing and loudness matching 12/40: song_name.wav
```

这能更准确地说明工具正在处理哪一首歌，而不是假装卡在 90%。


## v3.7.4 修复

修复“保存并重新生成 XML”按钮重复执行完整 Generate 流程的问题。
...
这会非常耗时，而且对于只修改 Marker / 段落设置的场景是多余的。

v3.7.4 起，该按钮改为：

```text
保存段落设置
只重写 output/RadioInfo_CN.xml
不重新生成 fmod_ready_wav
不重新执行音量匹配
```
...
```text
保存并重写 XML / Save and rewrite XML
```

如果用户更换了音乐文件、重新导入 Extract、修改了映射关系，仍然应使用完整的 `③ Generate / ③ 最终生成`。
...
- “保存并重写 XML”完成弹窗不再提及 `fmod_ready_wav` 或音量匹配；
- “导入 Extract”完成弹窗只提示下一步点击 Generate；
- 映射表提示改为普通用户无需修改；
- 音频错误列表现在会正确换行显示。
