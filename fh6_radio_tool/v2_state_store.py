...
def default_db_path(project_root: Path | None = None) -> Path:
    root = Path(project_root) if project_root else runtime_root()
...
class StateStore:
    ...
    def clear_all_assignments(self) -> int:
        """Remove all pending slot replacement assignments.

        Used after restoring game files from backup/initial state.  Restoring the
        files should also clear the tool-side replacement plan; otherwise the
        next one-click run can silently apply stale assignments again.
        Track profiles and saved marker presets are intentionally kept.
        """
        with self.connect() as conn:
            row = conn.execute("SELECT COUNT(*) AS n FROM slot_assignments").fetchone()
            count = int(row["n"] if row else 0)
            conn.execute("DELETE FROM slot_assignments")
        return count

    def clear_assignments_for_station(self, station_name: str) -> int:
        """Remove pending replacement assignments for one station."""
        with self.connect() as conn:
            row = conn.execute("SELECT COUNT(*) AS n FROM slot_assignments WHERE station_name=?", (station_name,)).fetchone()
            count = int(row["n"] if row else 0)
            conn.execute("DELETE FROM slot_assignments WHERE station_name=?", (station_name,))
        return count
